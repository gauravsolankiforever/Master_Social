...
class _CreateVideoStoryState extends State<CreateVideoStory> {
  ...
  void initState() {
    ...
    videoPlayerController = VideoPlayerController.file(widget.videoFile).initialize().then((value) => setState(() {}))as VideoPlayerController;
...
class _CreateVideoThumbnailState extends State<CreateVideoThumbnail> {
  ...
  Widget build(BuildContext context) {
    ...
    VideoPlayer(controller).cornerRadiusWithClipRRect(defaultAppButtonRadius) : Image.asset(ic_video, height: 20, width: 20, fit: BoxFit.cover);
  }
}
