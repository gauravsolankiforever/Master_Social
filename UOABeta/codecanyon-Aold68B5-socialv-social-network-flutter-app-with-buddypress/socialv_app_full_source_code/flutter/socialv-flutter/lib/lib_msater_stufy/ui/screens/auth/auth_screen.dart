...
class _SignInPageState extends State<_SignInPage> {
                  ...
                  onPressed: () {
                    if (_formKey.currentState!.validate()) {
                      if(_loginController.text == 'demoapp' && _passwordController.text == 'demoapp') {
                        preferences!.setBool('demo', true);
                        _bloc.add(LoginEvent(_loginController.text, _passwordController.text,false));
                      }else {
                        _bloc.add(LoginEvent(_loginController.text, _passwordController.text,false));
                      }
                    }
                  ...
                  AppButton(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        6.width,
                        Text("signinWithGoogle", style: secondaryTextStyle(), maxLines: 1, overflow: TextOverflow.ellipsis).flexible(),
                      ],
                    ).center(),
                    elevation: 1,
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Text(
                              ...
                              maxLines: 1, overflow: TextOverflow.ellipsis).flexible(),
                        ],
                      ),
                      elevation: 1,
                      color: context.cardColor,
  ...
  void googleSignIn() async {
    ...
    await service.signInWithGoogle().then((res) async {
      print("googleSignIn >>>>>>>>>>>>>>> "+res.values.toString());

     var response = LoginResponse.fromJson(await handleResponse(await buildHttpResponse(APIEndPoint.socialLogin, request: res as Map, method: HttpMethod.POST, isAuth: true)));
      ...
      print("googleSignIn >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.$response");



      // _bloc.add(
      //   RegisterEvent(
      //     response.userDisplayName.validate(),
      //     response.userEmail.validate(),
      //     response.userDisplayName.validate(),
      //   ),
      // );
