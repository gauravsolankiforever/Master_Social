...
Map<String, String> buildHeaderTokens(bool requiredNonce) {
  ...
  if (appStore.token.isNotEmpty) header.putIfAbsent(HttpHeaders.authorizationHeader, () => 'Bearer ${appStore.token}');
...
Future<Response> buildHttpResponse(
  ...
  if (await isNetworkAvailable()) {
    var headers = buildHeaderTokens(requiredNonce);
    ...
    if (method == HttpMethod.POST) {
      ...
      response = await delete(Uri.parse(url), headers: headers, body: jsonEncode(request));
